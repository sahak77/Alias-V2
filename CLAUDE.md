...
This repo is a **single VS Code workspace** containing two projects (the mobile app and the backend) plus the code they share.
...
| `contracts/` | **Shared Zod contracts** — the single source of truth for wire shapes, relative-imported by both projects as `../contracts` (`@alias/contracts`). *Scaffolded (tsup-built package; backend depends on it via `file:../contracts`).* | — |
...
> **When working in a project, read that project's `CLAUDE.md`** — `Alias-V2/` (mobile) and `Alias-V2-beckend/` (backend) each own their stack, commands, and standards. This file only covers what spans both. They are plain sibling folders (no npm-workspaces tooling); the shared `contracts/` package is consumed via relative-path import (`../contracts`, depended on as `file:../contracts`).
...
2. **One contract source.** Wire shapes are defined once in `contracts/` as Zod schemas. The app imports them for `z.infer` types; the server validates against them (via `nestjs-zod`). Never duplicate or fork a wire shape; never derive it from DB tables. The server re-validates everything — the contract is a *shape* convention, not a security boundary.
...
- Duplicate or fork a wire shape — define it once in `contracts/`.
