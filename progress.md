...
- ✅ **Word-pack multi-select** entry + combined-pool count — Setup lists the library as selectable chips (backed by `usePackStore`), builds the pool from `resolveSelectedPacks` (starter fallback), and shows the combined word count; a "New pack" shortcut opens the editor.
...
**Packs, content & data:**
- ⬜ **Client API / connection layer** *(prerequisite for download/AI/onboarding — NOT for the offline Pack Editor)* — the seam exists but is **unused today**: `src/lib/apiClient.ts` (typed fetch wrapper), base URL via `EXPO_PUBLIC_API_URL` (`app.config.ts` → `src/lib/config.ts`, currently the `https://api.example.com` placeholder), TanStack Query + provider, and `@alias/contracts` imported. To activate: (a) implement **OFFLINE-normalization** — fold `fetch` rejections/network failures into the shared `OFFLINE`/`NETWORK_UNAVAILABLE` envelope code (release-gate requirement, CLAUDE.md), (b) set a real base URL (localhost dev / Railway prod), (c) adopt the `useQuery`/`useMutation`-in-`features/*/api/` pattern the apiClient comment prescribes. No endpoint call exists in the app yet.
- 🟡 Local **Pack store + My-Packs library** — ✅ `usePackStore` (first-class `Pack` library: bundled starter + custom packs + next-game selection; own AsyncStorage key + schema-versioned hydrate that re-validates against the contract; offline-safe `resolveSelectedPacks`; wired into launch hydration). ✅ **My-Packs management screen** (`/packs`, from Home): `FlatList` of custom packs → tap to edit, delete-with-confirm, empty state, hydration gate (starter is always-available + not listed). ⬜ **Import** (QR/file). ⬜ Move the corpus to SQLite `expo-sqlite` once it outgrows a JSON blob.
- ⬜ **Download official packs** *(app side — backend ✅, see §3.3 (5))* — an "official / Discover" list in the Pack Library backed by a `useListPacks` query over `GET /v1/packs[?locale=]`; on install, fetch the blob **client-direct from the summary's `downloadUrl`** (R2/CDN — backend off the data path), gunzip → **verify `contentHash`** → Zod-validate against the `Pack` contract → persist as a local `source:'downloaded'` pack; show per-pack download/installed/update-available state. Degrades softly when offline (the bundled starter always plays).
  - ↳ **Backend:** ✅ `GET /v1/packs` + the R2 client-direct download model are done (§3.3 (5)). **Remaining content-ops (not code):** build the official-pack word blobs, upload them to R2 (`packs/{contentHash}.json.gz`), and update the seed's placeholder `content_hash`/`r2_key` (the deferred "R2 blob path" noted in `seed.ts`) — until then `downloadUrl` points at a non-existent object. Community **Discover / publish / ratings / reports** stays the deferred `v2` write side (§3.3 (6)).
...
- ✅ **Pack Editor — create a custom pack by hand** *(app-only; offline; no backend)* — `/pack-editor` screen + Home "Word packs" entry: pick the pack's **`locale`** (launch set) + title, enter words one-per-line (paste to **bulk-add**), **dedupe** (normalized), save a **`source:'custom'`** pack into `usePackStore` — immediately playable + selectable in Setup. Edit (`?id`) waits for hydration; delete is confirmed; the new-pack id is stable per session (double-tap-safe). Pure parse/build helper (`customPack.ts`) + screen + 6 tests. *(Remaining: per-word taboo/hint fields, edit-existing entry point via the My-Packs screen, fork-on-edit for imported packs.)*
  - ↳ **Backend:** none — fully on-device (no endpoint, no network). *(The AI-assisted create path is the separate AI Pack Generator, which uses `POST /v1/generate`.)*
