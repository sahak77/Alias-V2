...
| Backend service | 🟡 v2 schema authored; `/v1/generate` + `/v1/content-policy` live (env-gated); rest deferred |
...
- ✅ `POST /v1/generate` **live** (full AI proxy: provider + spend cap + attestation + content gate — §3.3.2) and `GET /v1/content-policy/:locale` **live** (R2/CDN OTA read path — §3.3.1); `GET /health` live.
