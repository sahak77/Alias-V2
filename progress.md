...
- ⬜ Local **Pack store + My-Packs library** (browse/select/create/import; introduce the first-class `Pack` entity + `source` provenance — `builtin`/`downloaded`/`custom`/`ai`/`imported`; persisted in its own AsyncStorage key, separate from the game-session store; move the corpus to SQLite `expo-sqlite` once it outgrows a JSON blob). Unblocks the Setup word-pack multi-select (§2.2) and is the home for the two add-words features below.
...
- ⬜ **Pack Editor — create a custom pack by hand** *(app-only; the lowest-dependency "add words" path — needs NO backend + NO client API layer)* — a new editor screen + `usePackStore` entry that: picks the pack's **`locale`** (the word language) and title/cover; adds / edits / deletes word cards (word + optional description, taboo terms, hint); supports **bulk-paste** + **dedupe** (normalized via the shared `infra/normalize.ts` so `contentHash` matches the server's); validates against the `Pack`/`Card` contract; saves a local **`source:'custom'`** pack that's immediately playable offline and selectable in Setup. Editing an **`imported`** pack forks a copy (imported packs are immutable). Optional AI-assist hooks into the AI Pack Generator below.
